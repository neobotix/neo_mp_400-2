...
def execution_stage(
    ...
    enable_docking = str(docking.perform(context)).lower()

    # If the parameter file is not provided, use the default one based on the robot type and docking condition
    if not params:
        if enable_docking == "true":
            params = os.path.join(
                get_package_share_directory('neo_mp_400-2'),
                'configs', 'navigation',
                'docking_navigation.yaml')
        else:
            params = os.path.join(
                get_package_share_directory('neo_mp_400-2'),
                'configs', 'navigation',
...
def generate_launch_description():
    ...
    declare_docking_cmd = DeclareLaunchArgument(
            'enable_docking', default_value='False',
            description='Enable docking for robot'
        )
    ...
    launch_desc.add_action(declare_docking_cmd)

    context_arguments = [use_sim_time, autostart, namespace,
                         use_multi_robots, head_robot, use_amcl, map_dir, param_dir, use_rviz, docking]
